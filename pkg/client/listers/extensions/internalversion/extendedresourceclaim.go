...
type ExtendedResourceClaimLister interface {
	...
	List(selector labels.Selector) (ret []*extensions.ExtendedResourceClaim, err error)
...
// List lists all ExtendedResourceClaims in the indexer.
func (s *extendedResourceClaimLister) List(selector labels.Selector) (ret []*extensions.ExtendedResourceClaim, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*extensions.ExtendedResourceClaim))
	})
	return ret, err
}
...
type ExtendedResourceClaimNamespaceLister interface {
	...
	Get(name string) (*extensions.ExtendedResourceClaim, error)
...
// List lists all ExtendedResourceClaims in the indexer for a given namespace.
func (s extendedResourceClaimNamespaceLister) List(selector labels.Selector) (ret []*extensions.ExtendedResourceClaim, err error) {
...
// Get retrieves the ExtendedResourceClaim from the indexer for a given namespace and name.
func (s extendedResourceClaimNamespaceLister) Get(name string) (*extensions.ExtendedResourceClaim, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(extensions.Resource("extendedresourceclaim"), name)
	}
	return obj.(*extensions.ExtendedResourceClaim), nil
}
